...
def compliance_agent(state: dict) -> dict:
    ...
    for check in checks:
        print(f"  [PASS] {check['check']}: {check['result']} - {check['details']}")
    ...
    print(f"[Compliance Agent] Claim AUTO-APPROVED & SUBMITTED.\n")
...
def human_escalation(state: dict) -> dict:
    ...
    """
    print("[Human Escalation] Low confidence detected - escalating to human reviewer...")
    ...
    for reason in escalation_reasons:
        print(f"  [WARNING] Escalation reason: {reason}")
    ...
    for notif in notifications:
        print(f"  [NOTIF] {notif['channel']}: {notif['status']} - {notif['message']}")
    ...
    print(f"[Human Escalation] Case ESCALATED. Pending human review.\n")
