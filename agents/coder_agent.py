    ...
    def validate_icd10_codes(cls, v):
        ...
        for code in v:
            ...
            if icd10_pattern.match(code):
                ...
                print(f"  [WARNING] Invalid ICD-10 code format rejected: {code}")
        return validated
...
def coder_agent(state: dict) -> dict:
    ...
    try:
        ...
        print("  [LLM] Calling Claude Sonnet 4...")
        response = llm.invoke(messages)
        raw_output = response.content.strip()
        print(f"  [DATA] Raw AI response: {raw_output[:200]}...")
        ...
        print(f"  [SUCCESS] Codes: {medical_codes}")
        ...
        # --- RAG Simulation ---
        rag_context = (
            "Rule R101: Acute sinusitis (J01) must have symptom onset within 4 weeks.\n"
            "Rule R102: Laterality must be explicitly documented; use unspecified if absent.\n"
            "Rule R103: PHI masking must be verified before LLM submission."
        )

        # --- Validator Simulation ---
        is_modified = False
        corrections = []
        if len(medical_codes) > 3:
            is_modified = True
            corrections.append("Clipped excessive ICD-10 suggestions as per CMS rules.")
            medical_codes = medical_codes[:3]

        validation_result = {
            "is_modified": is_modified,
            "status": "Self-corrected output format for compliance." if is_modified else "Output matches all clinical formats.",
        ...
        print(f"  [ERROR] Coder Agent Error: {str(e)}")
